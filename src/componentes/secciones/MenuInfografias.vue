...
JDVista
  ...
  .carrusel-container(ref="contenedorRef")
    Carousel3d(
      ...
      Slide(
        v-for="(opcion, idx) in listaInfografias"
        :key="idx"
        :index="idx"
      )
...
const { listaInfografias } = storeToRefs(almacenInfografias);
...
const idResaltado = ref<number>(Math.floor(_.size(listaInfografias.value) / 2));
const opcionResaltada = computed(() => _.nth(listaInfografias.value, idResaltado.value));

function seleccionar(id: string) {
  almacenInfografias.seleccionarInfografia(id);
  seccion('infografia');
}
